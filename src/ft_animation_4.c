...
/*   Updated: 2023/04/10 17:41:07 by rteles           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_str(t_all *all, int number, int color, int x)
{
	...
	mlx_string_put(all->ptr, all->win, x,
		all->hgt * (all->max_y + 1.3), color, str);
	free(str);
}
...
void	put_str_plus(t_all *all, int number, int color, int x)
{
	...
	mlx_string_put(all->ptr, all->win, x,
		all->hgt * (all->max_y + 1.3), 0x00000000, str);
	free(str);
...
void	animation_time(t_all *all, int count)
{
	...
	else if ((count == 5000) || (count == 15000))
	{
		if (all->m.time == -1)
			ft_lives(all);
		if (all->m.time > 10)
			put_str(all, all->m.time, 0x00FFFFFF, all->wth * 1.5);
		else if (all->m.time <= 10)
